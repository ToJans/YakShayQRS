...
namespace MinimalisticCQRS.Specs.Domain
{
    ...
    public class AccountTransfers
    {
        ...
        public void DefineScope()
        {
            ...
            SUT = new SpecBus(mvr);
            mvr.Register<Account>();
            mvr.Register<AccountTransferSaga>();
        }

        [TestMethod]
        public void Transfering_an_amount_from_a_to_b_should_succeed_if_all_valid()
        {
            SUT.GivenEvent(x => x.AccountRegistered(OwnerName: "Tom Janssens", AccountNumber: "123-456789-01", AccountId: "account/1"));
            SUT.GivenEvent(x => x.AmountDeposited(Amount: 100m, AccountId: "account/1"));
            SUT.GivenEvent(x => x.AccountRegistered(OwnerName: "Liesbeth kint", AccountNumber: "223-456789-01", AccountId: "account/2"));
